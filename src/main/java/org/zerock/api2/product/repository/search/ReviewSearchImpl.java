...
public class ReviewSearchImpl extends QuerydslRepositorySupport implements ReviewSearch {
    ...
    public Page<Review> listByProduct(Long pno, Pageable pageable) {
        ...
        JPAQuery<Review> query = from(review);
        query.leftJoin(review.images, image); //review의 images를 image로 봐!
        query.where(review.product.pno.eq(pno));
//        query.where(image.ord.eq(0)); //이미지가 0인 애들
        ...
        JPQLQuery<Tuple> tupleQuery = query.select(review.rno, review.score, image);//위에서 image
